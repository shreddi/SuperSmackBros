...
func get_transition(delta):
	...
	match state:
		...
		states.STRONG_DASH_ATTACK:
			if parent.frame == 0:
				parent.STRONG_DASH_ATTACK()
			if parent.frame == 1:
					parent.velocity.y -= 400
					if parent.velocity.x > 0:
						parent.velocity.x += 450
					elif parent.velocity.x < 0:
						parent.velocity.x -= 450
			if parent.frame >= 1:
				parent.velocity.y += 40
				if parent.velocity.x > 0:
					parent.velocity.x -= 15
				elif parent.velocity.x < 0:
					parent.velocity.x += 15
		...
		states.UP_SPECIAL:
			parent.special = false
			AIRMOVEMENT()
			if parent.frame == 3:
				parent.velocity.y = -2500
				parent.UP_SPECIAL()
...
func Ledge():
	if not state_includes([states.DAIR, states.HITSTUN]):
...
func enter_state(new_state, old_state):
	match new_state:
		states.STAND:
			parent.play_animation("mario_idle")
			parent.state.text = str("STAND")
		states.DASH:
			parent.play_animation("mario_run")
			parent.state.text = str("DASH")
		states.RUN:
			parent.play_animation("mario_run")
			parent.state.text = str("RUN")
		states.TURN:
			parent.play_animation("mario_turn")
			parent.state.text = str("TURN")
		states.JUMP_SQUAT:
			parent.play_animation("mario_jump_squat")
		...
		states.HITSTUN:
			parent.play_animation("mario_hurt")
		...
		states.GRAB:
			parent.play_animation('mario_grab')
			parent.state.text = str('GRAB')
		states.THROW:
			parent.play_animation('mario_throw')
			parent.state.text = str('THROW')
		states.DOWN_TILT:
			parent.play_animation('mario_dtilt')
			parent.state.text = str('DOWN_TILT')
		states.UP_TILT:
			parent.play_animation('mario_utilt')
			parent.state.text = str('UP_TILT')
		states.FORWARD_TILT:
			parent.play_animation('mario_ftilt')
			parent.state.text = str('FORWARD_TILT')
		states.UP_SMASH:
			parent.play_animation('mario_up_smash')
			parent.state.text = str('UP_SMASH')
		states.FORWARD_SMASH:
			parent.play_animation('mario_fsmash')
			parent.state.text = str('FORWARD_SMASH')
		states.DOWN_SMASH:
			parent.play_animation('mario_down_smash')
			parent.state.text = str('DOWN_SMASH')
		states.DASH_ATTACK:
			parent.play_animation("mario_dash_attack")
			parent.play_animation("DASH_ATTACK")
		states.STRONG_DASH_ATTACK:
			parent.play_animation("mario_strong_dash_attack")
			parent.play_animation("STRONG_DASH_ATTACK")
		states.BAIR:
			parent.play_animation('mario_bair')
			parent.state.text = str('BAIR')
		states.FAIR:
			parent.play_animation('mario_fair')
			parent.state.text = str('FAIR')
		states.DAIR:
			parent.play_animation('mario_dair')
			parent.state.text = str('DAIR')
		states.UAIR:
			parent.play_animation('mario_uair')
			parent.state.text = str('UAIR')
		states.NAIR:
			parent.play_animation('mario_nair')
			parent.state.text = str('NAIR')
		states.NEUTRAL_SPECIAL:
			parent.play_animation('mario_grab')
			parent.state.text = str('NEUTRAL_SPECIAL')
		states.UP_SPECIAL:
			parent.play_animation('mario_upspecial')
			parent.state.text = str('UP_SPECIAL')
		states.SHIELD:
			parent.play_animation('mario_sheild')
